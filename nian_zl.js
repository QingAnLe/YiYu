				 ...
				 if (err) {
				  ...
				  console.log(`${$.name} API请求失败，请检查网路重试`)
					  ...
					  if (data && data.data['bizCode'] === 0) {
						  let koiInviteIdData =  data.data.result.homeMainInfo.koiInviteId;
						  tz='您的好友助力码为：${koiInviteIdData}'
    ...
    $.post(taskPostUrl('nian_getHomeData', {inviteId: inviteId}), async (err, resp, data) => {
      try {
        if (err) {
          ...
          data = JSON.parse(data);
		    if (data && data.data['bizCode'] === 0) {
			  let koiInviteIdData =  data.data.result.homeMainInfo.guestInfo.msgForStatus;
			  console.log(`助力结果：${koiInviteIdData}`)
		  }else{
			console.log(`账号被风控，无法参与活动`)
		  }
        }
