...
const inviteCodes = [
  `cAxZdTXtIbPevg-cXVT7vnybLJOVZZh42uplQ1QlU3YHuTXSu7fWKFMrg6Q@cAxZdTXtYMSfmEm3e2_PmnbruXBoYq7_d5Qs97BzRkLWyRaERLLczbA`,
  `cAxZdTXtIbPevg-cXVT7vnybLJOVZZh42uplQ1QlU3YHuTXSu7fWKFMrg6Q@cAxZdTXtYMSfmEm3e2_PmnbruXBoYq7_d5Qs97BzRkLWyRaERLLczbA`,
];
      ...
      try {
        if (err) {
          ...
          if (data && data.data['bizCode'] === 0) {
            ...
            let koiInviteIdData =  $.userInfo.koiInviteId;		 
		    console.log(`您的好友助力码为：${koiInviteIdData}\n`);
		    message += `您的好友助力码为：${koiInviteIdData}\n`
          } else {
            $.secretp = null
...
function collectScore(inviteId) {
  let temp = {
    "inviteId": "-1"
  }
...
function requireConfig() {
  return new Promise(resolve => {
    ...
    if ($.isNode() && process.env.JDNIAN_ZHULICODES) {
      if (process.env.JDNIAN_ZHULICODES.indexOf('\n') > -1) {
        shareCodes = process.env.JDNIAN_ZHULICODES.split('\n');
      } else {
        shareCodes = process.env.JDNIAN_ZHULICODES.split('&');
      }
    }
