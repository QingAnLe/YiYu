...
if(!$.isNode()&&xyzdhd.indexOf("\n") ==-1){
    xyzdurlArr.push($.getdata('xyzdurl'))
    xyzdhdArr.push($.getdata('xyzdhd'))
    xyzdbodyArr.push($.getdata('xyzdbody'))
} else {
    if($.isNode()){
    if (process.env.XYZD_HD && process.env.XYZD_HD.indexOf('\n') > -1) {
        xyzdhd = process.env.XYZD_HD.split('\n');
    } else {
        xyzdhd = [process.env.XYZD_HD]
    };
    if (process.env.XYZD_URL && process.env.XYZD_URL.indexOf('\n') > -1) {
        xyzdurl = process.env.XYZD_URL.split('\n');
    } else {
        xyzdurl = [process.env.XYZD_URL]
    };
    if (process.env.XYZD_BODY && process.env.XYZD_BODY.indexOf('\n') > -1) {
        xyzdbody = process.env.XYZD_BODY.split('\n');
    } else {
        xyzdbody = [process.env.XYZD_BODY]
    };	
 ...
 console.log(` ============= 您共提供${xyzdhdArr.length}个幸运赚点账号 =============`);
}
